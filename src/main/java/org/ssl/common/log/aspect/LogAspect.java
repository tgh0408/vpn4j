package org.ssl.common.log.aspect;
...
import org.ssl.common.core.utils.NetUtils;
...
import org.ssl.common.log.enums.BusinessStatus;
...
public class LogAspect {
    ...
    private void setRequestValue(JoinPoint joinPoint, OperLogEvent operLog, String[] excludeParamNames) {
        ...
        if (MapUtil.isEmpty(paramsMap) && Strings.CS.equalsAny(requestMethod, HttpMethod.PUT.name(), HttpMethod.POST.name(), HttpMethod.DELETE.name())) {
    ...
    private boolean isInnerIp(String ip) {
        return (NetUtils.isIPv4(ip) && NetUtils.isInnerIP(ip)) ||
                (NetUtils.isIPv6(ip) && NetUtils.isInnerIPv6(ip));
    }
}
